...
function bindEvents() {
  ...
  els.allBlocosBtn.addEventListener('click', listAllBlocos);
  ...
  els.loadPlanningBase.addEventListener('click', renderPlanningFromBase);
...
function parseCSV(text) {
  ...
  return lines
    .map((line) => {
      ...
      bloco.nome_bloco = bloco.nome_bloco || bloco.nome || 'Bloco sem nome';
      bloco.endereco_concentracao = bloco.endereco_concentracao || bloco.endereco || '';
      bloco.bairro = bloco.bairro || '';
      bloco.data = bloco.data || '';
      bloco.hora_concentracao = bloco.hora_concentracao || bloco.hora || '';
...
async function geocodeBlocoAddress(bloco) {
  const rawAddress = [bloco.endereco_concentracao, bloco.bairro, 'Brasil'].filter(Boolean).join(', ');
...
function renderResults(list, mode) {
  ...
  list.forEach((bloco) => {
    ...
    node.querySelector('.bloco-address').textContent = `${bloco.endereco_concentracao || 'Endereço não informado'} (${bloco.bairro || 'Bairro não informado'})`;
...
function renderPlanningList(list) {
  ...
  if (!list.length) {
    els.planningList.innerHTML = '<p class="card">Você ainda não salvou blocos no planejamento.</p>';
    return;
  }
  ...
  list.forEach((bloco) => {
    ...
    item.innerHTML = `
      ...
      <p><strong>🕒 Concentração:</strong> ${bloco.data || '--'} ${bloco.hora_concentracao || '--'}</p>
      <p><strong>📍 Endereço:</strong> ${bloco.endereco_concentracao || 'Não informado'} (${bloco.bairro || 'Bairro não informado'})</p>
      <p>${bloco.metro || '🚇 Metrô: a calcular'}</p>
    `;
...
async function renderPlanningFromBase() {
  await ensureDataLoaded();
  const allBlocos = await enrichBlocosWithCoordinates(state.blocos);
  const plan = getPlanningList();
  const existingIds = new Set(plan.map((item) => item._id));

  allBlocos.forEach((bloco) => {
    if (!existingIds.has(bloco._id)) {
      plan.push({ ...bloco, metro: findNearestMetro(bloco), _id: bloco._id, savedAt: new Date().toISOString() });
    }
  });
